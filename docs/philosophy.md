...
## PlanGate が OSS として配布するもの

PlanGate を OSS として外部に公開する立場から見ると、本プロジェクトが提供する価値は次の 1 文に集約されます。

> **PlanGate の OSS 価値は「AI 開発の安全な型を配布すること」であって、「作者の運用ログを高度化すること」ではない。**

この方針は 2026-05-14〜05-15 の Claude × Codex × Gemini による 5 ラウンド戦略ディスカッション（[`docs/working/discussions/`](./working/discussions/)）で確認されたものです。
...
外部 OSS 利用者の最大摩擦は「機能不足」ではなく **「どこまで使えばよいか分からない」** です。PlanGate は次の段階で必要な分だけ採用できます。

- **Level 1** — plan 承認だけ
- **Level 2** — handoff まで
- **Level 3** — hooks / validate
- **Level 4** — metrics / outcome review（**自己進化軸はここから実質開始**）
- **Level 5** — eval / timeline（experimental / advanced）

自己進化機能は Level 4 以降に置き、quickstart には混ぜません。

### 意図的に取り入れないもの

業界主流であっても PlanGate が **意図的に切り捨てている** 設計判断:

- 自動学習 / skill 自動更新（OSS では誤学習の回復コストが高い）
- OpenTelemetry GenAI semconv 必須準拠（汎用 observability platform を目指さない）
- LLM-as-a-Judge multi-judge consensus（single judge + human rationale で十分）
