...
出力は [`schemas/eval-baseline.schema.json`](../../../schemas/eval-baseline.schema.json) に準拠（D-2 test で検証）。privacy: §3 Allowed のみ、ファイルパス / コマンド出力 / プロンプト等の §4 Forbidden は schema レベルで物理的に阻止。

## 8. Snapshot

機械可読 baseline は同梱の [`2026-05-04-baseline.json`](./2026-05-04-baseline.json) に格納。schema は [`schemas/eval-baseline.schema.json`](../../../schemas/eval-baseline.schema.json) を参照（v8.6.0 PR4 で正式化、D-2 test で毎回検証）。schema の概要：
