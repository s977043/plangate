...
> **Status**: v2（**Implementation: Done** — Issue #157 / TASK-0048 で 3 hook を実装。dry-run / strict / bypass の 3 モード）
...
> 実装: [`scripts/hooks/check-c3-approval.sh`](../../scripts/hooks/check-c3-approval.sh) / [`check-handoff-elements.sh`](../../scripts/hooks/check-handoff-elements.sh) / [`check-fix-loop.sh`](../../scripts/hooks/check-fix-loop.sh)
...
## 4. 実装（Issue #157 / TASK-0048 で完了）

EH-2 / EHS-2 / EHS-3 を以下の通り実装:

| Hook | 種別 | 実装 |
|------|------|------|
| EH-2（C-3 未承認 exec block）| Claude Code PreToolUse hook | [`scripts/hooks/check-c3-approval.sh`](../../scripts/hooks/check-c3-approval.sh) |
| EHS-2（handoff 必須 6 要素）| CLI（手動 / WF-05 で呼び出し）| [`scripts/hooks/check-handoff-elements.sh`](../../scripts/hooks/check-handoff-elements.sh) |
| EHS-3（fix loop 上限超過）| CLI（V-1 fix loop 内で increment / check）| [`scripts/hooks/check-fix-loop.sh`](../../scripts/hooks/check-fix-loop.sh) |
...
[`.claude/settings.example.json`](../../.claude/settings.example.json) を `.claude/settings.json` にコピーすると PreToolUse hook（EH-2）が有効化される。EHS-2 / EHS-3 は CLI ツールとして workflow 内で呼び出す。

### 4.4 テスト

- 単体: `sh tests/hooks/run-tests.sh` → 12 件 PASS
- 統合: `sh tests/run-tests.sh` の TA-06 で hook 子テストを呼び出し

### 4.5 EH-1 / EH-3〜7 / EHS-1 の status

本 PBI（#157）で実装した EH-2 / EHS-2 / EHS-3 以外は **未実装（Spec のみ）**。次の Hook 実装 PBI で順次対応。
