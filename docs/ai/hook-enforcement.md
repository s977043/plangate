...
> **Status**: v4（**Implementation: 8/10 hooks Done** — #157 で EH-2 / EHS-2 / EHS-3、#169 セッション A で EH-1 / EH-3、#169 セッション B で EH-4 / EH-5 / EH-6 を追加。3 mode 設計）
...
## 4. 実装（#157 で 3 hook、#169 セッション A で +2、セッション B で +3、計 8 hook）
...
| **EH-4**（test-cases.md なし V-1 block）| CLI（V-1 前で呼び出し）| [`scripts/hooks/check-test-cases.sh`](../../scripts/hooks/check-test-cases.sh) | #169 / TASK-0057 |
| **EH-5**（検証ログなし PR 作成 block）| CLI（PR 作成前で呼び出し）| [`scripts/hooks/check-verification-evidence.sh`](../../scripts/hooks/check-verification-evidence.sh) | #169 / TASK-0057 |
| **EH-6**（scope 外ファイル編集検知）| PreToolUse hook + CLI | [`scripts/hooks/check-forbidden-files.sh`](../../scripts/hooks/check-forbidden-files.sh) | #169 / TASK-0057 |
...
**残未実装**: EH-7（branch protection 連携）/ EHS-1（V-3 必須化）— 次セッション C
...
[`.claude/settings.example.json`](../../.claude/settings.example.json) を `.claude/settings.json` にコピーすると PreToolUse hook（**EH-1 + EH-2 + EH-3 + EH-6**）+ SessionStart（gh-pin-account）が有効化される。EH-4 / EH-5 / EHS-2 / EHS-3 は CLI ツールとして workflow 内で呼び出す。

### 4.4 テスト

- 単体: `sh tests/hooks/run-tests.sh` → **33 件 PASS**（#157 で 12 + #169 セッション A で +9 + セッション B で +12）
- 統合: `sh tests/run-tests.sh` の TA-06 で hook 子テストを呼び出し

### 4.5 EH-7 / EHS-1 の status

#157 で 3 hook + #169 セッション A で 2 hook + セッション B で 3 hook = 計 **8 hook 実装済**。残 2 hook（EH-7 branch protection 連携 / EHS-1 V-3 必須化）は **未実装（Spec のみ）**、Issue #169 のセッション C で対応予定。
