...
- 以下 6 agents を `.claude/agents/` から `plugin/plangate/agents/` に配置（コピー）:
  - `workflow-conductor`
  - `spec-writer`
  - `implementer`
  - `linter-fixer`
...
**実態調査で判明**: 当初計画の 6 agents のうち `test-engineer` と `release-manager` は `.claude/agents/` に存在しない。6 agents に絞って移植する。
...
- [ ] 6 agents 全てが `plugin/plangate/agents/` 配下に存在する
...
- [ ] plan → C-1 セルフレビュー → exec → L-0 → V-1 → PR のフローが plugin 内 6 agents で完結する
...
| 6 agents のうち隠れた固有依存を持つ agent がある | Medium | 各 agent の全文レビューを C-1 セルフレビューで実施 |
...
- 6 agents は現行 `.claude/agents/` で動作している
