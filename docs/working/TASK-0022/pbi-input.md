...
2. **C-1: セルフレビュー** — Plan 7項目 + ToDo 5項目 + TestCases 3項目 の 15 項目チェック（standard モード）
3. **C-2: 外部AIレビュー** — standard モードではスキップ可能（モード分類マトリクス参照）
4. **C-3: 人間レビュー** — 三値ゲート（APPROVE / CONDITIONAL / REJECT）

**モード判定**: `standard`（plan.md の Mode判定セクションで確定）
