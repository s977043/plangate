...
graph TD
    ...
    Mode -->|"full/critical"| V2["AI V-2: コード最適化 + テスト再実行"]
...
| **V-2**: コード最適化 | AI | 冗長コード削減・可読性向上(full/criticalモード) | 最適化済みコード+テスト再実行 |
...
- **V-2(コード最適化)** -- 冗長コード削減・可読性向上(full/criticalモード)
...
4. **V-2(コード最適化・full/criticalモード)** -- 冗長コード削減・可読性向上。動作を変えない改善に限定。最適化後にテスト再実行
...
> - **full**: 標準plan → C-1 → C-2 → TDD+並列 → L-0 → V-1 → V-2 → V-3 → PR → C-4
