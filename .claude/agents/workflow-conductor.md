...
## 起動前提とケイパビリティ・フォールバック（TASK-0072）

conductor 委譲は **委譲可能環境での最適化** であり、ハード前提ではない
（[`core-contract.md`](../../docs/ai/core-contract.md) §5-bis 実行環境不変条件）。

exec 開始時に **`Task` 利用可否をツール存在検査で判定**し、実行者を分岐する:

- **委譲可能（top-level 等）** → conductor が orchestrate、implementer へ委譲（従来どおり・最適経路）
- **委譲不可（subagent コンテキスト等）= `delegation_unavailable`** → **メイン/単一エージェントが直接 Implementer を実行するのが既定の正規フロー**。これは「停止して人間に報告」ではなく **人間介入不要の自動降格**。conductor の Iron Law（実装しない）は破られない（実行者がメインへ移るだけで、降格は委譲不能環境に限定）
- 判定不能 → 安全側＝直接実行

> 旧挙動（subagent 起動を検知したら報告して停止し呼び出し元が代行）は
> **撤廃**。停止→人間介入は field で恒常デッドロック化したため、
> 自動フォールバックを正規フロー化した（#237/#238/#239/#234-E）。
> 関連: `AGENT_LEARNINGS.md`（2026-05-16）, [`contracts/execute.md`](../../docs/ai/contracts/execute.md) Error taxonomy
...
V-1: 受け入れ検査
  ...
  ↓
V-2: コード最適化（high-risk/criticalモード、役割7が判定）
