...
plan.mdのタスク規模に基づき、V-2/V-4のスキップ判定を自動実行する。

#### モード判定ロジック

```text
plan.mdの `## Mode判定` セクションを確認:
- 「ライトモード」にチェック → L-0 → V-1 → V-3 → PR → C-4
- 「フルモード」にチェック → L-0 → V-1 → V-2 → V-3 → V-4 → PR → C-4
- セクションなし → 以下の自動判定:
  - 変更ファイル数 <= 1 かつ バグ修正/設定変更 → ライト
  - それ以外 → フル
```
