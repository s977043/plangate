...
conductor は **メイン会話（top-level）から起動** されることを前提とする。conductor が subagent コンテキストで起動されると `Agent`/`Task` ツールが使えず（"Task is not available inside subagents"）、implementer への委譲が構造的に不可能になる。

- conductor が subagent 起動だと判明したら、**実装を自前で行わず**（Iron Law 維持）、その旨を呼び出し元に報告して停止する
- 呼び出し元（メイン）は、conductor を subagent 化せず自身が orchestrator を代行し、implementer を直接派遣する
- 関連: `AGENT_LEARNINGS.md`（2026-05-16 conductor top-level 起動）
