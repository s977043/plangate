...
> **conductor 起動方式（重要）**: exec は workflow-conductor に委譲するが、conductor は **top-level（メイン会話）から起動** すること。conductor が subagent 化すると `Agent`/`Task` 不可で implementer 委譲が破綻する。subagent 起動になった場合はメインが orchestrator を代行し implementer を直接派遣する（`AGENT_LEARNINGS.md` 2026-05-16）。
