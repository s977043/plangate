...
> [!WARNING]
> **新規導入では、Option A / Option B のどちらか一方を選ぶことを推奨します。**
> plugin と `.claude/` のデュアル運用は移行・高度なカスタマイズ用途では可能ですが、同名 Skill / command の解決順に注意してください。
> plugin 側を明示する場合は `plangate:` prefix を使います。詳細は [plugin 移行ガイド](docs/plangate-plugin-migration.md) を参照してください。
...
テストは `plangate validate --dir` の 4 fixture（complete-task / missing-approval / stale-plan-hash / broken-pbi）に加え、v8.1.0 で追加された `validate --mode`、`review`、`exec` のケースを検証します。
